...
with st.sidebar:
    ...
    bankroll = st.number_input("Bankroll (USDC)", 100.0, 10_000_000.0, 10_000.0, step=500.0)
    kelly_lambda = st.select_slider("Kelly fraction (λ)",
                                    options=[0.10, 0.25, 0.50, 1.00], value=0.25)
    max_frac = st.slider("Max bankroll per market", 0.01, 0.50, 0.10, 0.01)
    edge_threshold = st.slider("Min edge to trade (¢)", 0.005, 0.15, 0.03, 0.005)
    edge_se = st.slider("Edge std error (model uncertainty)", 0.005, 0.10, 0.02, 0.005)
    fee = st.number_input("Per-side fee", 0.0, 0.01, 0.0005, step=0.0001, format="%.4f")
    min_hours = st.number_input("Min hours to expiry", 0.0, 72.0, 0.5, step=0.5)
